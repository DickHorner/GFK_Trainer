...
export interface LlmEvaluationResult {
  exerciseId: string;
  fieldId: string;
  present: boolean;  // Whether the GFK element is identifiable
  score: 0 | 1 | 2 | 3;  // Quality score: 0=none, 1=mixed, 2=good, 3=excellent
  ...
  exampleRewrite: string;  // 1-2 sentence example (not a full rewrite)
  explainers: string[];  // Highlights of strengths and areas to develop
  meta?: {
    fieldType?: string;
    pairIndex?: number;
  };
 ...
 */
export function parseAndRepairJsonResponse(raw: string): LlmEvaluationResult | null {
  const trimmed = raw.trim();
  
  // Try direct parse first
  try {
    return JSON.parse(trimmed) as LlmEvaluationResult;
  } catch (_e) {
  ...
  if (jsonMatch) {
    try {
      return JSON.parse(jsonMatch[1]) as LlmEvaluationResult;
    } catch (_e) {
  ...
  try {
    const parsed = JSON.parse(repaired) as LlmEvaluationResult;
 ...
 */
export function validateEvaluationResult(result: any): result is LlmEvaluationResult {
  return (
 ...
 */
export function sanitizeEvaluationResult(result: LlmEvaluationResult): LlmEvaluationResult {
